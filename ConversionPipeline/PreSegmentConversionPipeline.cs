...
namespace OpenCCNET.ConversionPipeline
{
    ...
    /// 基于预分词的转换管道
    /// </summary>
    public class PreSegmentConversionPipeline : IConversionPipeline
    {
        ...
        public PreSegmentConversionPipeline(string text)
        {
            // 只分词一次
        ...
        private PreSegmentConversionPipeline(List<string> phrases)
        {
            _phrases = phrases;
        }
        ...
        public IConversionPipeline ConvertBy(params IDictionary<string, string>[] dictionaries)
        {
            ...
            {
                result = _phrases.Select(phrase => ConvertPhrase(phrase, dictionaries)).ToList();
            }

            return new PreSegmentConversionPipeline(result);
        }
